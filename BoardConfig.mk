# BoardConfig.mk — Xiaomi 17 Ultra / nezha
...
# 5. AVB
# Verified:
#   bootloader state must be checked before testing
#   ro.boot.verifiedbootstate=green on collected rooted system
#   ro.boot.vbmeta.device_state=locked on collected rooted system
...
BOARD_QTI_DYNAMIC_PARTITIONS_SIZE := 15290335232
...
#   android.hardware.security.keymint.IKeyMintDevice/strongbox   — device-provided StrongBox service
#   android.hardware.gatekeeper.IGatekeeper/default              — gatekeeper-rust-service-qti
#   android.hardware.weaver.IWeaver/default                      — Goodix eSE Weaver service
...
TW_SKIP_SECONDARY_FBE_USERS := true
...
TW_Y_OFFSET := 0
TW_H_OFFSET := 0
# Keep TWRP settings off /persist. On HyperOS, /persist maps to
# /mnt/vendor/persist; recovery-created files there can leave SELinux
# labels that make Goodix secure_element crash after rebooting Android.
TW_PERSIST_DIR := /data/recovery/TWRP

# -----------------------------------------------------------------------------
# 13. Input / touch / haptics
# Verified:
#   Xiaomi_Touch_Input_0 = /dev/input/event8
#   non-touch devices include uinput-xiaomi
#   Haptics use qcom-hv-haptics input FF device.
# -----------------------------------------------------------------------------
TW_CUSTOM_TOUCH_DEVICE := "/dev/input/event8"
...
# -----------------------------------------------------------------------------
TW_LOAD_VENDOR_MODULES := "qmi_helpers.ko qcom_glink.ko qcom_glink_smem.ko qcom_smd.ko rproc_qcom_common.ko qcom_pdr_msg.ko pdr_interface.ko qcom_sysmon.ko qcom_q6v5.ko qcom_ramdump.ko qcom_va_minidump.ko qcom_pil_info.ko qcom_q6v5_pas.ko q6_pdr_dlkm.ko q6_notifier_dlkm.ko snd_event_dlkm.ko gpr_dlkm.ko spf_core_dlkm.ko adsp_loader_dlkm.ko q6_dlkm.ko pcie-pdc.ko pci-msm-drv.ko mhi.ko wcd_usbss_i2c.ko usb_f_gsi.ko dwc3-msm.ko repeater.ko redriver.ko ipam.ko rmnet_mem.ko smem-mailbox.ko cfg80211.ko mac80211.ko wlan_firmware_service.ko cnss_prealloc.ko cnss_utils.ko cnss_nl.ko cnss_plat_ipc_qmi_svc.ko cnss2.ko qca_cld3_peach_v2.ko qti_pmic_glink.ko qti_battery_charger.ko panel_event_notifier.ko gh_irq_lend.ko xiaomi_touch.ko synaptics_tcm2.ko swr_dlkm.ko mca_sysfs.ko mca_event.ko mca_log.ko mca_parse_dts.ko mca_charge_mievent.ko mca_protocol_class.ko mca_protocol_qc_class.ko mca_platform_bc12_class.ko mca_platform_buckchg_class.ko mca_strategy_class.ko mca_adsp_glink.ko mca_qcom_subpmic_proxy.ko leds-qcom-flash.ko leds-qpnp-vibrator-ldo.ko qcom-hv-haptics.ko swr_haptics_dlkm.ko qsee_ipc_irq_bridge.ko hdcp_qseecom_dlkm.ko gsim.ko spi-msm-geni.ko stm_st54se_gpio.ko smcinvoke_dlkm.ko stm_nfc_i2c.ko"
