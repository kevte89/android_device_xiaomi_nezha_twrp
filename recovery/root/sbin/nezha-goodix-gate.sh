...
ensure_gsea_sysfs() {
    ...
    if [ ! -e "$GSEA_COS_STATE" ] || [ ! -e "$GSEA_STRONGBOX_STATE" ]; then
        ...
        if [ "$(getprop twrp.nezha.crypto_route)" = "normal_590" ]; then
            log_msg "GSEA sysfs missing; continuing Goodix chain for normal_590 test"
            return 0
        fi
        return 1
    fi
...
detect_route() {
    ...
    route=fallback
    case "$hwver" in
        5.9.7*) route=leica_597 ;;
        5.9.0*|5.19.0*) route=normal_590 ;;
    esac
    case "$model" in
        *25128PNA1C*) route=leica_597 ;;
        *2512BPNDAC*) route=normal_590 ;;
    esac
...
prepare_tmp_state() {
    ...
    if [ -d /data/vendor ] || [ -d /data/media ]; then
        ...
        chown -R system:system /data/vendor/qwes 2>/dev/null || true
        ...
        chmod 0770 /data/vendor/qwes 2>/dev/null || true
        restorecon -RFv /data/vendor/goodix /data/vendor/secure_element \
            /data/vendor/qwes /mnt/vendor/persist/goodix /mnt/vendor/persist/data \
            >> "$LOG" 2>&1 || true
    fi
}
...
if ! ensure_secure_element_firmware before_recovery_qwesd; then
    log_msg "secure-element firmware disappeared before recovery.qwesd"
    exit 0
fi
stop vendor.qwesd
log_msg "start recovery.qwesd"
start recovery.qwesd
log_msg "wait_stable_running recovery.qwesd"
if ! wait_stable_running recovery.qwesd "$qwes_stable" "$qwes_limit"; then
    setprop twrp.nezha.goodix_gate_error recovery_qwesd_failed
    log_msg "recovery.qwesd failed state=$(getprop init.svc.recovery.qwesd)"
    exit 0
fi
